...
void main()
{
	...
	if (use_normal_mapping){
		...
		Diffuse = texture(diffuse_map, fs_in.texcoord).rgb * max(dot(normalize(fs_in.normal), L), 0.0f);
		Specular = specular * pow( max(dot(reflect(-L, normalize(fs_in.normal)), V), 0), shininess) ;
	}
