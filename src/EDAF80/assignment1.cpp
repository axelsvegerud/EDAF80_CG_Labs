...
int main()
{
	...
	/* Create a ring shape using parametric_shapes::createCircleRing() function.
	Arguments:
	(a) radius radius from the centre to the middle of the cross-section;
	(b) spread_length length of the cross-section;
	(c) circle_split_count the number of times the angle for the circle should be split;
	(d) spread_split_count the number of times the lines going out from the centre should be split; */

	bonobo::mesh_data ring_shape = parametric_shapes::createCircleRing(1.6f, 0.8f, 80u, 8u);

	CelestialBody sun = CelestialBody(sphere, &celestial_body_shader, sun_texture);
	sun.set_scale(glm::vec3(1.0f));
	sun.set_spin(glm::radians(0.0f), glm::two_pi<float>() / 6.0f);
	//sun.set_orbit(2.0f, glm::radians(-20.0f), 0.0f);
	//sun.set_ring(ring_shape, &celestial_ring_shader, saturn_ring_texture);

	// *** PLANETS ***

	// Setup for Earth:
	CelestialBody earth = CelestialBody(sphere, &celestial_body_shader, earth_texture);
	//earth.set_scale(glm::vec3(0.25f));
	earth.set_scale(glm::vec3(0.05f));
	earth.set_spin(glm::radians(23.0f), glm::two_pi<float>() / 3.0f);
	earth.set_orbit(4.0f, glm::radians(7.2f), glm::two_pi<float>() / 20.0f);

	// Setup for Moon:
	CelestialBody moon = CelestialBody(sphere, &celestial_body_shader, moon_texture);
	//moon.set_scale(glm::vec3(0.25f));
	moon.set_scale(glm::vec3(0.01f));
	moon.set_spin(glm::radians(6.7f), glm::two_pi<float>() / 90.0f);
	//moon.set_orbit(3.5f, glm::radians(-66.0f), glm::two_pi<float>() / 1.3f);
	moon.set_orbit(0.2f, glm::radians(-66.0f), glm::two_pi<float>() / 1.3f);

	// Setup for Mercury:
	CelestialBody mercury = CelestialBody(sphere, &celestial_body_shader, mercury_texture);
	mercury.set_scale(glm::vec3(0.02f));
	mercury.set_spin(glm::radians(2.0f), glm::two_pi<float>() / 180.0f);
	mercury.set_orbit(2.0f, glm::radians(3.4f), glm::two_pi<float>() / 4.0f);

	// Setup for venus:
	CelestialBody venus = CelestialBody(sphere, &celestial_body_shader, venus_texture);
	venus.set_scale(glm::vec3(0.05f));
	venus.set_spin(glm::radians(177.0f), glm::two_pi<float>() / 600.0f);
	venus.set_orbit(3.0f, glm::radians(3.9f), glm::two_pi<float>() / 12.0f);

	// Setup for mars;
	CelestialBody mars = CelestialBody(sphere, &celestial_body_shader, mars_texture);
	mars.set_scale(glm::vec3(0.03f));
	mars.set_spin(glm::radians(25.0f), glm::two_pi<float>() / 3.0f);
	mars.set_orbit(5.0f, glm::radians(5.7f), glm::two_pi<float>() / 36.0f);

	// Setup for jupiter:
	CelestialBody jupiter = CelestialBody(sphere, &celestial_body_shader, jupiter_texture);
	jupiter.set_scale(glm::vec3(0.5f));
	jupiter.set_spin(glm::radians(3.1f), glm::two_pi<float>() / 1.0f);
	jupiter.set_orbit(13.0f, glm::radians(6.1f), glm::two_pi<float>() / 220.0f);

	// Setup for saturn:
	CelestialBody saturn = CelestialBody(sphere, &celestial_body_shader, saturn_texture);
	saturn.set_scale(glm::vec3(0.4f));
	saturn.set_spin(glm::radians(27.0f), glm::two_pi<float>() / 1.2f);
	saturn.set_orbit(16.0f, glm::radians(5.5f), glm::two_pi<float>() / 400.0f);
	saturn.set_ring(ring_shape, &celestial_ring_shader, saturn_ring_texture, { 1.0f, 1.25f });

	// Setup for uranus:
	CelestialBody uranus = CelestialBody(sphere, &celestial_body_shader, uranus_texture);
	uranus.set_scale(glm::vec3(0.2f));
	uranus.set_spin(glm::radians(98.0f), glm::two_pi<float>() / 2.0f);
	uranus.set_orbit(18.0f, glm::radians(6.5f), glm::two_pi<float>() / 1680.0f);

	// Setup for neptune
	CelestialBody neptune = CelestialBody(sphere, &celestial_body_shader, neptune_texture);
	neptune.set_scale(glm::vec3(0.2f));
	neptune.set_spin(glm::radians(28.0f), glm::two_pi<float>() / 2.0f);
	neptune.set_orbit(19.0f, glm::radians(6.4f), glm::two_pi<float>() / 3200.0f);
	...
	while (!glfwWindowShouldClose(window)) {
		...
		/* Pseudocode for Depth-first search: ( https://en.wikipedia.org/wiki/Depth-first_search )
		...
		*/
		...
		std::stack<CelestialBody*> stack({ &sun });
		std::stack<glm::mat4> mat_stack({ glm::mat4(1.0f) });

		CelestialBody* curr_node = NULL;
		glm::mat4 curr_mat;
