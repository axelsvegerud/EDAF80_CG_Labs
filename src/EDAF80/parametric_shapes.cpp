...
parametric_shapes::createSphere(float const radius, unsigned int const longitude_split_count, unsigned int const latitude_split_count) {
	...
	for (unsigned int i = 0u; i < longitude_slice_vertices_count; i++) {
		...
		for (unsigned int j = 0u; j < latitude_slice_vertices_count; j++) {
			...
			// Tangent:
			// Divide the tangent by sin_phi -> Remove sin_phi from equation.
	...
	// Create index array:
	auto index_sets = std::vector<glm::uvec3>(2u * (static_cast<float>(latitude_slice_edges_count)) * (static_cast<float>(longitude_slice_edges_count)));
	...
	unsigned int const minor_split_count)
{
	...
	// rA -> major_radius
	// rB -> minor_radius
	// res_theta -> minor_split_count
	// res_phi -> major_split_count

	auto const vertices_nb = minor_split_count * major_split_count;
	auto vertices = std::vector<glm::vec3>(vertices_nb);
	auto normals = std::vector<glm::vec3>(vertices_nb);
	auto texcoords = std::vector<glm::vec3>(vertices_nb);
	auto tangents = std::vector<glm::vec3>(vertices_nb);
	...
	float theta = 0.0f;
	float d_theta = glm::two_pi<float>() / (static_cast<float>(minor_split_count - 1u));
	float phi = 0.0f;
	float d_phi = glm::two_pi<float>() / (static_cast<float>(major_split_count - 1u));
	...
	for (unsigned int i = 0u; i < minor_split_count; i++){
		...
		for (unsigned int j = 0u; j < major_split_count; j++){
			...
			// tangent
			auto t = glm::vec3(-minor_radius * sin_theta * cos_phi, -minor_radius * sin_theta * sin_phi, -minor_radius * cos_theta);
			//t = glm::normalize(t);
			tangents[index] = t;

			// binormal
			auto b = glm::vec3(-(major_radius + minor_radius *cos_theta) * sin_phi, (major_radius + minor_radius * cos_theta) * cos_phi, 0);
			//b = glm::normalize(b);
			binormals[index] = b;
	...
	// generate indices iteratively
	index = 0u;
	for (unsigned int i = 0u; i < minor_split_count - 1u; ++i)
	{
		for (unsigned int j = 0u; j < major_split_count - 1u; ++j)
		{
			
			indices[index] = glm::uvec3(major_split_count * i + j,
				major_split_count * (i + 1) + j,
				major_split_count * i + j + 1u);
			++index;

			indices[index] = glm::uvec3(major_split_count * (i + 1) + j,
				major_split_count * (i + 1) + j + 1u,
				major_split_count * i + j + 1u);
			++index;
		}
	}
